...
import { type KeyState, type VehicleTelemetry } from "./types";
...
function detectWheels(carModel: THREE.Group): WheelDetection {
...
export function VehicleModel({
  ...
  const wheelDetectionRef = useRef<WheelDetection | null>(null);
  if (wheelDetectionRef.current == null) {
    wheelDetectionRef.current = detectWheels(carModel);
  }
  ...
  useFrame(({ camera }, delta) => {
    ...
    const wheelDetection = wheelDetectionRef.current;
    if (!wheelDetection) {
      return;
    }
    ...
    rearLeftPos.copy(rearCenter).addScaledVector(right, -carTransform.rearTrackHalf);
    rearRightPos.copy(rearCenter).addScaledVector(right, carTransform.rearTrackHalf);
    ...
    telemetry.speed = Math.abs(velocityRef.current);
    telemetry.braking = keys.s;
    telemetry.position.copy(vehicle.position);
    telemetry.forward.copy(forward);
    telemetry.rearLeft.copy(rearLeftPos);
    telemetry.rearRight.copy(rearRightPos);
  });
